...
public class ParamController {
    ...
    /** Público: parametrías para los filtros del catálogo. Cambian poco: cacheable 5 min. */
    @GetMapping("/api/param-groups")
    public ResponseEntity<List<GroupResponse>> publicList() {
        List<GroupResponse> body = service.findAll().stream().map(GroupResponse::from).toList();
